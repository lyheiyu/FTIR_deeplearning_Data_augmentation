...
if __name__ == '__main__':
    ...
    # not use polymerName, waveLength, intensity, polymerID = utils.parseData4th('dataset/FourthDataset3.csv')
    ...
    # cmTotal = np.zeros((11, 11))
        x_train, x_test, y_train, y_test = train_test_split(intensity, polymerID, test_size=0.3, random_state=inum)
        ...
        for n in range(len(PN)):
            ...
            indexForIntensity = idexes.take(range(0, 50), mode='wrap')
            ...
            for item in intensityRandomLoop:
                ...
                #baseline1 = normalized_wns*random.randint(-500,500)*0.01 +  random.randint(-5,5) # linear baseline
            ...
            #data2=random.sample(data,20)

            # y_add2=random.sample(y_add,50)
...
#
    cmTotal = cmTotal / m
    #utils.plot_confusion_matrix(cmTotal, PN, 'Baseline augmentation KNN')
    #utils.plot_confusion_matrix(cmTotal, PN, 'Baseline augmentation MLP')
    #
    #utils.plot_confusion_matrix(cmTotal, PN, 'Baseline augmentation SVM')
    ...
    for item in data_plot:
        plt.plot(waveLength,item,c='b')
    plt.title('Baseline method',font2)
    plt.xlim(max(waveLength),min(waveLength)-100)
    plt.ylim(-5,5)
    plt.xticks(fontsize=30)
    plt.yticks(fontsize=30)
    # plt.xticks()
    # plt.yticks(labelsize=15)
    # ax[0].plot(x, y1, '-k')
    # for item in data_plot:
    #
