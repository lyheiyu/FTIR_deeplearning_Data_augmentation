...
if __name__ == '__main__':
    ...
    for n in range(0,11):
        ...
        print('```````',np.dot(intensityForLoop,Vector[1].T))
        ...
        for ttt in range(100):
            ...
            for nnn in range(len(Vector[0])):
                if Vector[0][i] > 0.03:
                    noise.append(random.randint(4,5)*0.1)
                if Vector[0][i] <=0.03 :
                    noise.append(0)
        ...
        for ttt in range(100):
            ...
            for nnn in range(len(Vector[1])):
                if Vector[1][i] > 0.01:
                    noise2.append(random.randint(2, 4) * 0.1)
                if Vector[1][i] <= 0.01:
                    noise2.append(0)
        ...
        datas = np.concatenate((datas, dataFromMovingSmooth), axis=0)
