...
if __name__ == '__main__':
    ...
    polynomial_columns = [np.ones_like(waveLength)]
    for j in range(1, 3):
        polynomial_columns.append(normalized_wns ** j)
    reference=np.mean(intensity,axis=0)
    reference=np.array(reference,dtype='float')
    X = np.stack((reference, *polynomial_columns), axis=1)
    print('X',X)
    A = np.dot(np.linalg.pinv(np.dot(X.T, X)), X.T)
    print('A',A.shape)
    print("normlized_wns",normalized_wns)
    print("normlized_columns",polynomial_columns)
    normalized_wns=np.array(normalized_wns)
    polynomial_columns=np.array(polynomial_columns)
    print("normlized_wns", normalized_wns.shape)
    print("normlized_columns", polynomial_columns.shape)
    new=np.ones((3,3))
    new2=np.zeros((3,3))
    newadd=[[-0.2,-0.3,-0.4],[-0.2,-0.3,-0.4]]
    newadd=np.array(newadd)
    new3=np.concatenate((new,new2),axis=0)
    new3=np.concatenate((new3,newadd),axis=0)
    ...
    if(np.any(mask)):
        n_resamples = mask.sum()
        new3[6][mask] = np.random.uniform(0, new3[6][mask],
                                               n_resamples)
    print(new3[6])
    # self.X = np.stack((self.ref, *self.polynomial_columns), axis=1)
    # self.A = np.dot(np.linalg.pinv(np.dot(self.X.T, self.X)), self.X.T)
