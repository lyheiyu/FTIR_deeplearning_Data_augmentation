...
if __name__ == '__main__':
    ...
    for itr in range(1,2):
        ...
        #         loss='mse'
        #     )
        #     datafortraining = np.concatenate((dataSquare2[i], dataSquare3[i]), axis=0)
        #     datafortraining = np.concatenate((datafortraining, dataSquare4[i]), axis=0)
        #     datafortraining2 = np.concatenate((dataSquare3[i], dataSquare4[i]), axis=0)
        #     datafortraining2 = np.concatenate((datafortraining2, dataSquare2[i]), axis=0)
        #     datafortraining3 = np.concatenate((dataSquare4[i], dataSquare4[i]), axis=0)
        #     datafortraining3 = np.concatenate((datafortraining3, dataSquare4[i]), axis=0)
        #     autoencoder.fit([datafortraining,datafortraining2],datafortraining3,
        #                     epochs = 50,
        #                     batch_size = 32,
        #                     shuffle = True)
        ...
        #     intensity1 = autoencoder.predict([dataSquare3[i], dataSquare2[i]])
        #     intensity2 = autoencoder.predict([dataSquare2[i], dataSquare2[i]])
        #     intensity3 = autoencoder.predict([dataSquare3[i], dataSquare3[i]])
        #     x_train = np.concatenate((x_train, intensity0), axis=0)
        #     x_train = np.concatenate((x_train, intensity1), axis=0)
        #     x_train = np.concatenate((x_train, intensity2), axis=0)
        #     x_train = np.concatenate((x_train, intensity3), axis=0)
        ...
        t = classification_report(y_test, y_pre, target_names=PN, output_dict=True)
        utils.mkdir(model.__class__.__name__ + 'AE_square_report')
        SVM_report = pd.DataFrame(t)
        print(SVM_report)
        SVM_report.to_csv(
            model.__class__.__name__ + 'AE_square_report/' + model.__class__.__name__ + 'AE_square_report' + str(i) + '.csv')
        t_report += t
        print(t.__class__)
    print(scoreTotal / v)
    ...
    utils.plot_confusion_matrix(cmTotal, PN, 'square augmentation SVM '+str(test_size))
    statisticForMetrics = pd.DataFrame(statisticForMetrics)
    statisticForMetrics.to_csv('square augmentation SVM 50%.csv')