...
def emsc(spectra: np.ndarray, wavenumbers: np.ndarray, order: int = 2,
    ...
    A = np.dot(linalg.pinv(np.dot(X.T, X)), X.T)
...
from FTIR_Least_square_method import generatedataBySperateLS
from FTIR_fit_least_square import generatedataBySperateLSforEach,\
    generatedataBySperateLSforEach3,generatedataBySperateLSforEach4,generatedataBySperateLSforEach5,\
    generatedataBySperateLSforEach6,generatedataBySperateLSforEach7,generatedataBySperateLSforEach8
import gc
...
if __name__ == '__main__':
    ...
    from FTIR_dataGenerateByConcentratedNet import TwoDimensionalAugmentation
    waveLength=np.array(waveLength,dtype='float32')
    ...
    for seedNum in range(20):
        ...
        #     indexForIntensity=idexes.take(range(0,10),mode='wrap')
        #     intensityforRandomLoop=intensityForLoop[indexForIntensity]
        #     #data2D = intensityForLoop[indexForIntensity]
        #     data2D, label = generatedataBySperateLSforEach6(waveLength, intensityforRandomLoop, n)
        #     # data2D, label = LSTMPolynomiaRegression(intensityforRandomLoop, n)
        #     peaknosieforloop=[]
        #
    ...
    #utils.plot_confusion_matrix(cmTotal, PN, 'Segmented augmentation SVM')
    utils.plot_confusion_matrix(cmTotal, PN, 'ESMA SVM')
    #utils.plot_confusion_matrix(cmTotal, PN, 'ESMA MLP')
    #utils.plot_confusion_matrix(cmTotal, PN, 'Original KNN')
    ...
    # for item in data2D:
    #     ax[1].plot(waveLength, item, 'b')
    # for item in data2D:
    #     ax.plot(waveLength, item, 'b')
    # plt.title('SA method', font2)
    # plt.xlim(max(waveLength), min(waveLength)-100)
    for item in augmentedSpectrum[0]:
        ax.plot(waveLength, item, 'b')
    plt.title('ESMA method', font2)
    plt.xlim(max(waveLength), min(waveLength)-100)
