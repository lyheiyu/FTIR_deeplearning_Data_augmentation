...
polymerName, waveLength, intensity, polymerID,x_each,y_each =utils.parseData2('D4_4_publication5.csv', 2, 1762)
latent_dim = 64
rf= readFile()
folderName = os.path.basename(os.getcwd())
# os.chdir(os.path.dirname(os.getcwd()))
indicesPS = [i for i, id in enumerate(polymerID) if id == 1]
dataset=intensity
print(len(intensity[0]))
TestSpec=intensity[indicesPS]
dataset = prepareSpecSet(TestSpec, transpose=False, addDimension=True)
specLength =len(intensity[0])
...
generator = keras.Sequential(
    [
        ...
        layers.LeakyReLU(alpha=0.2),
        layers.Conv1DTranspose(1761, kernel_size=4, strides=2, padding="same"),
        layers.LeakyReLU(alpha=0.2),
...
gan.fit(dataset, epochs=epochs, callbacks=[GANMonitor(wavenumbers=waveLength, latent_dim=latent_dim)])
