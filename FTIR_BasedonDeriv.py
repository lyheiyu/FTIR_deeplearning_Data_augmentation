...
if __name__ == '__main__':
    ...
    for seedNum in range(2):
        ...
        for n in range(0,11):
            ...
            x_train = cal_2nd_deriv(waveLength, x_train)
            ...
            print(deirves2.shape)
            deirves3 = np.array(deirves3)
            print(deirves3.shape)
            ...
            deirvesforloop = deirves[indexForIntensity]
            deirves2forloopindex = idexes.take(range(0, 200),mode='wrap')
            deirves2forloop2=deirves2[indexForIntensity]
            deirvesforloopindex = idexes.take(range(0, 200), mode='wrap')
            deirvesforloop3 = deirves3[indexForIntensity]
            ...
            for i in range(len(intensityforRandomLoop)):
                ...
                for eachj in range(len(deirvesforloop[i])):
                    if eachj>=len(deirvesforloop[i])-2:
                        noiseforeahc.append( random.randint(1,2)*deirvesforloop[i][eachj]
                                             )
                    else:
                        noiseforeahc.append( random.randint(1,2)*deirvesforloop[i][eachj]
                                             )

                intensityplusnoise.append(np.array(noiseforeahc)+random.randint(-6,6)*0.3)
            # for i in range(len(intensityforRandomLoop)):
            #     noiseforeahc = []
            #     for eachj in range(len(deirvesforloop[i])):
            #
            ...
            intensityplusnoise=intensityplusnoise[0:100]
    ...
    #x_train, x_test0, y_train, y_test0 = train_test_split(x_train, y_train, test_size=0.1, random_state=1)
        ...
        x_test = cal_2nd_deriv(waveLength, x_test)
