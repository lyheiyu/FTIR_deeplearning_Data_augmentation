...
class TestKeras:
    ...
    def plot_confusion_matrix(cm, labels_name, title):
        ...
        plt.show()
class ThreeDimensionalAugmentation(object):
    ...
    def generateData(self,num):
        ...
        d0 = add([encoded1, encoded2,encoded3])
        encoded = Dense(inputshape/2, activation='relu')(d0)
        decoded = Dense(inputshape/2, activation='relu')(encoded)

        decoded = Dense(inputshape, activation='linear')(decoded)
        autoencoder = Model(inputs=[input1,input2,input3], outputs=decoded)  # 实例化
        autoencoder.compile(
            optimizer='adam',
            loss='mse'
        )
        autoencoder.fit([data2,data3,data4],data2,
                        epochs = 3,
                        batch_size = 512,
                        shuffle = True)

        intensity0=autoencoder.predict([data2, data3,data4])
...
if __name__ == '__main__':
    ...
    for itr in range(2,3):
        ...
        for i in range(0,12):
            ...
            autoencoder.fit([dataSquare2[i],dataSquare3[i],dataSquare4[i]],dataSquare2[i],
                            epochs = 30,
                            batch_size = 32,
                            shuffle = True)
            ...
            intensity0=autoencoder.predict([dataSquare2[i], dataSquare3[i],dataSquare4[i]])
    ...
    utils.plot_confusion_matrix(cmTotal, PN, '3D Concentrated_square augmentation MLP'+str(test_size))
    statisticForMetrics = pd.DataFrame(statisticForMetrics)
    statisticForMetrics.to_csv('3D Concentrated_square augmentation.csv')