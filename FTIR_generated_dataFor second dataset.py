import numpy

from utils import  utils
from keras.layers import Input,add
from keras.models import Sequential
from keras.layers import LSTM
from keras.layers import Dense
from keras.layers import RepeatVector
from keras.layers import TimeDistributed
from keras.models import Model
import numpy as np
import pandas  as  pd
import matplotlib.pyplot as plt

from sklearn.neural_network import  MLPClassifier
import  torch
from sklearn.neighbors import  KNeighborsClassifier
from sklearn.ensemble import RandomForestClassifier
from sklearn.preprocessing import MinMaxScaler
import keras.backend as K
from sklearn.metrics import recall_score
from sklearn.metrics import confusion_matrix
from sklearn.metrics import classification_report
from math import isnan
from sklearn.model_selection import cross_val_score
from sklearn import  svm
from sklearn.model_selection import train_test_split
from utils import utils
import tensorflow as tf
import random
import sys  # 导入sys模块
# sys.setrecursionlimit(30000)
from sklearn.preprocessing import normalize
MMScaler = MinMaxScaler()
def randomText(spectrumLength,m,n):
    length = spectrumLength
    if length < 1:
        return ''
    if length == 1:
        return 0
    if length == 2:
        return spectrumLength-1

    randomNumber= random.randint(0, length - 1)
    if randomNumber==m or randomNumber==n:
       print('random',randomNumber)
       return randomText(spectrumLength,m,n)
    return randomNumber
class secondDatasetGenerate():
    def __init__(self,polymerName,waveLength,intensity,polymerID):
        self.polymerName=polymerName
        self.waveLength=waveLength
        self.intensity=intensity
        self.polymerID=polymerID
    def generateDataByLSTM(self,data):
        for itr in range(0,10):
            x_train, x_test, y_train, y_test = train_test_split(self.intensity, self.polymerID, test_size=0.6, random_state=itr)
            input_img = 1761
            encoding_dim = 512
            input_img = Input(shape=(1761,))
            intensity = np.array(intensity, dtype=np.float32)
            x_train = np.array(x_train, dtype=np.float32)
            print(x_train.shape)
            PN = utils.getPN('D4_4_publication5.csv')
            y_e = []
            for it in y_train:
                if it not in y_e:
                    y_e.append(it)
            if len(y_e) < 12:
                continue
            y_e2 = []
            for itt in y_test:
                if itt not in y_e2:
                    y_e.append(itt)
            if len(y_e) < 12:
                continue
            datas = []
            for i in range(len(PN)):
                indicesPS = [j for j, id in enumerate(y_train) if id == i]
                data = x_train[indicesPS]
                datas.append(data)

            dataSquare = []
            dataSquare2 = []
            dataSquare3 = []
            dataSquare4 = []
            numPoly = 0
            ylabel = []
            for item in datas:

                data = []
                data2 = []
                data3 = []
                data4 = []
                for m in range(len(item)):

                    for n in range(len(item)):
                        data.append([item[m], item[n]])
                        data3.append(item[n])
                        data2.append(item[m])

                        rm = randomText(len(item), m, n)

                        print(len(item), m, n, rm)
                        data4.append(item[rm])

                        ylabel.append(numPoly)
                numPoly += 1

                dataSquare.append(data)
                dataSquare2.append(data2)
                dataSquare3.append(data3)
                dataSquare4.append(data4)
            ylabel = np.array(ylabel)




            for i in range(len(datas)):
                datas[i] = datas[i].astype('float64')
            dataSquare = np.array(dataSquare)
            dataSquare2 = np.array(dataSquare2)
            dataSquare3 = np.array(dataSquare3)
            print('dataSquare2 shape', dataSquare2.shape)
            print('dataSquare3 shape', dataSquare3.shape)
            for i in range(len(dataSquare)):
                dataSquare[i] = np.array(dataSquare[i], dtype=float)
            for i in range(len(dataSquare2)):
                dataSquare2[i] = np.array(dataSquare2[i], dtype=float)
            for i in range(len(dataSquare3)):
                dataSquare3[i] = np.array(dataSquare3[i], dtype=float)
            for i in range(len(dataSquare4)):
                dataSquare4[i] = np.array(dataSquare4[i], dtype=float)
            numForSquare = 0
            for i in range(0, 12):
                print('iiiiiiiiiiiiii', len(dataSquare2[i]))

                if len(datas[i]) >= 100:
                    continue
                numForSquare += 1

                inputs = Input(shape=(1, 1761))
                encoded = LSTM(1024)(inputs)
                decoded = RepeatVector(1)(encoded)
                decoded = LSTM(1761, return_sequences=True)(decoded)

                model = Model(inputs, decoded)

                model.compile(
                    optimizer='adam',
                    loss='mse'
                )
                print(dataSquare2[i].shape[0], dataSquare2[i].shape[1])
                dataSquare2[i] = dataSquare2[i].reshape((dataSquare2[i].shape[0], 1, dataSquare2[i].shape[1]))
                dataSquare4[i] = dataSquare4[i].reshape((dataSquare4[i].shape[0], 1, dataSquare4[i].shape[1]))
                datas[i] = datas[i].reshape((datas[i].shape[0], 1, datas[i].shape[1]))
                # dataSquare4[i] = dataSquare4[i].reshape((dataSquare4[i].shape[0], dataSquare4[i].shape[1], 1))
                model.fit(dataSquare2[i], dataSquare4[i],
                          epochs=50,
                          batch_size=32,
                          shuffle=True)
                # encoder.predict([dataSquare2[i],dataSquare3[i]])
                # print(encoder.predict([dataSquare2[i],dataSquare3[i]]))
                indicesPS = [j for j, id in enumerate(ylabel) if id == i]
                ylabel0 = ylabel[indicesPS]
                count = 200 - len(dataSquare2[i])
                if count >= len(dataSquare2[i]):
                    count = len(dataSquare2[i])
                intensity0 = model.predict(dataSquare2[i])
                np.random.shuffle(intensity0)
                # intensityShuffle=np.array(intensityShuffle)
                # print(intensityShuffle)
                intensityPlus = intensity0[0:count]
                ylabel0 = ylabel0[0:count]
                print('augmentation intensity shape', intensity0.shape)
                print('augmentation intensityPLus shape', intensityPlus.shape)
                intensityPlus = intensityPlus.reshape((intensityPlus.shape[0], intensityPlus.shape[2]))

                return intensityPlus,ylabel0

if __name__ == '__main__':

    polymerName, waveLength, intensity, polymerID = utils.parseDataForSecondDataset('new_SecondDataset.csv')
    sdg=secondDatasetGenerate(polymerName, waveLength, intensity, polymerID)

    PN =[]
    for item in polymerName:
        if item not in PN:
            PN.append(item)
    statisticForMetrics = []
    model = svm.SVC(C=0.3, kernel='linear', decision_function_shape='ovo')
    # model = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(128, 128), random_state=1)
    models = [model]
    cmTotal = np.zeros((5, 5))

    v = 0
    t_report = []
    scoreTotal = np.zeros(5)
    test_size=0.6
    for itr in range(1, 3):
        x_train, x_test, y_train, y_test = train_test_split(intensity, polymerID, test_size=test_size, random_state=itr)
        input_img = 1761
        encoding_dim = 512

        intensity = np.array(intensity, dtype=np.float32)
        x_train = np.array(x_train, dtype=np.float32)
        print(x_train.shape)
        y_e = []
        for it in y_train:
            if it not in y_e:
                y_e.append(it)
        print(y_e)
        if len(y_e) < len(PN):
            continue
        y_e2 = []
        for itt in y_test:
            if itt not in y_e2:
                y_e.append(itt)
        print(y_e2)
        if len(y_e) < len(PN):
            continue
        datas = []
        for i in range(len(PN)):
            indicesPS = [j for j, id in enumerate(y_train) if id == i]
            data = x_train[indicesPS]
            datas.append(data)
        # datas=np.array(datas)
        # for item in datas:
        #     print(item.shape)
        # print(datas.shape)
        dataSquare = []
        dataSquare2 = []
        dataSquare3 = []
        dataSquare4=[]
        numPoly = 0
        ylabel = []
        ylabelfordatas = []
        for item in datas:

            data = []
            data2 = []
            data3 = []
            data4=[]
            for m in range(len(item)):
                ylabelfordatas.append(numPoly)
                for n in range(len(item)):
                    data.append([item[m], item[n]])
                    data3.append(item[n])
                    data2.append(item[m])
                    rm = randomText(len(item), m, n)

                    print(len(item), m, n, rm)
                    data4.append(item[rm])
                    ylabel.append(numPoly)

            numPoly += 1
            # data=np.array(data,dtype=np.float32)
            # print(data.shape)
            dataSquare.append(data)
            dataSquare2.append(data2)
            dataSquare3.append(data3)
            dataSquare4.append(data4)
        ylabel = np.array(ylabel)

        # dataSquare=np.array(dataSquare)
        # print('1111111111111111',dataSquare.shape)

        x_test = np.array(x_test, dtype=np.float32)
        y_train = np.array(y_train, dtype=np.float32)
        y_test = np.array(y_test, dtype=np.float32)
        # # x_train, x_test, y_train1, y_test1 = map(torch.tensor, (x_train, x_test, y_train, y_test))
        # print(x_train,x_test)
        for i in range(len(datas)):
            datas[i] = datas[i].astype('float64')
        dataSquare = np.array(dataSquare)
        dataSquare2 = np.array(dataSquare2)
        dataSquare3 = np.array(dataSquare3)
        dataSquare4 = np.array(dataSquare4)
        print('dataSquare2 shape', dataSquare2.shape)
        print('dataSquare3 shape', dataSquare3.shape)
        for i in range(len(dataSquare)):
            dataSquare[i] = np.array(dataSquare[i], dtype=float)
        for i in range(len(dataSquare2)):
            dataSquare2[i] = np.array(dataSquare2[i], dtype=float)
        for i in range(len(dataSquare3)):
            dataSquare3[i] = np.array(dataSquare3[i], dtype=float)
        for i in range(len(dataSquare4)):
            dataSquare4[i] = np.array(dataSquare4[i],dtype=float)
        # for i in range(len(datas)):
        #     datas[i] = datas[i].astype('float64')
        # y_train = np.concatenate((y_train, y_add), axis=0)
        # x_train, x_test, y_train1, y_test1 = map(torch.from_numpy, (x_train, x_test, y_train, y_test))
        # for i in range(2, len(PN),2):
        for i in range(1, 3):
            print('iiiiiiiiiiiiii', i)
            input1 = Input(shape=(len(waveLength),))
            input2 = Input(shape=(len(waveLength),))
            encoded1 = Dense(1024, activation='relu')(input1)
            encoded2 = Dense(1024, activation='relu')(input2)
            # d0 = tf.keras.layers.concatenate([encoded1, encoded2], axis=1)

            d0 = add([encoded1, encoded2])
            encoded = Dense(1024, activation='relu')(d0)
            # encoded = Dense(512, activation='relu')(encoded)
            # encoder_output = Dense(encoding_dim)(encoded)


            decoded = Dense(1024, activation='relu')(encoded)
            # decoded = Dense(2048, activation='relu')(decoded)
            decoded = Dense(len(waveLength), activation='tanh')(decoded)
            autoencoder = Model(inputs=[input1, input2], outputs=decoded)
            # encoder.predict([dataSquare2[i],dataSquare3[i]])
            # print(encoder.predict([dataSquare2[i],dataSquare3[i]]))
            autoencoder.compile(
                optimizer='adam',
                loss='mse'
            )
            autoencoder.fit([dataSquare2[i], dataSquare3[i]], dataSquare2[i],
                            epochs=50,
                            batch_size=32,
                            shuffle=True)
            # encoder.predict([dataSquare2[i],dataSquare3[i]])
            # print(encoder.predict([dataSquare2[i],dataSquare3[i]]))
            indicesPS = [j for j, id in enumerate(ylabel) if id == i]
            ylabel0 = ylabel[indicesPS]
            count = 200 - len(datas[i])
            if count >= len(dataSquare2[i]):
                count = len(dataSquare2[i])
            intensity0 = autoencoder.predict([dataSquare2[i], dataSquare3[i]])
            np.random.shuffle(intensity0)
            # intensityShuffle=np.array(intensityShuffle)
            # print(intensityShuffle)
            intensityPlus = intensity0[0:count]
            ylabel0 = ylabel0[0:count]
            print('augmentation intensity shape', intensity0.shape)
            print('augmentation intensityPLus shape', intensityPlus.shape)
            x_train = np.concatenate((x_train, intensityPlus), axis=0)

            y_train = np.concatenate((y_train, ylabel0), axis=0)

            xtofile = pd.DataFrame(x_train, y_train)
            print(xtofile)
            utils.mkdir(model.__class__.__name__ + 'seconddatasetFortraining'+str(1-test_size))


            xtofile.to_csv(
                model.__class__.__name__ + 'seconddatasetFortraining'+str(1-test_size)+'/' + model.__class__.__name__ + 'second training set' + str(
                    i) + '.csv')

            # ylabelfordatas=numpy.array(ylabelfordatas)
            # indicesPS = [j for j, id in enumerate(ylabelfordatas) if id == i]
            # ylabel0 = ylabelfordatas[indicesPS]
            # print(ylabelfordatas)
            # print(ylabel0)
            # print(ylabel0.shape)
            #
            # intensity0 = autoencoder.predict(datas[i])
            # print(intensity0.shape)
            # x_train = np.concatenate((x_train, intensity0), axis=0)
            # y_train = np.concatenate((y_train, ylabel0), axis=0)

        print(PN)
        model.fit(x_train, y_train)
        print('model fit')
        y_pre = model.predict(x_test)

        print('model pre',y_pre)
        y_p = []
        for y_p_e in y_pre:
            if y_p_e not in y_p:
                y_p.append(y_p_e)
        if len(y_p) < len(PN):
            continue
        utils.printScore(y_test, y_pre)
        #

        t = classification_report(y_test, y_pre, target_names=PN, output_dict=True)
        cm = confusion_matrix(y_test, y_pre)
        # model = tS.spectrumSVM(x_train, y_train, 0.3, 'linear', 'ovo')
        y_pre = model.predict(x_test)

        cm = confusion_matrix(y_test, y_pre)
        cmtemp = np.zeros((len(PN), len(PN)))
        if len(cm) < len(PN):
            print('cm equals 11', len(cm))
            continue
            # for i in range(len(cm)):
            #     cmtemp[i]=np.append(cm[i],[0],axis=0)
            #     # cmtemp[i]+=cm[i]
            # cmtemp[11,11]=2
            # print('tempCM；',cmtemp)
            # cmTotal+=cmtemp
            # continue
        cmTotal = cmTotal + cm
        scores = utils.printScore(y_test, y_pre)
        v += 1
        scoreTotal += scores
        print('v:', v)

        t = classification_report(y_test, y_pre, target_names=PN, output_dict=True)
        utils.mkdir(model.__class__.__name__ + 'second square AE')
        SVM_report = pd.DataFrame(t)
        print(SVM_report)
        SVM_report.to_csv(
            model.__class__.__name__ + 'second square AE/' + model.__class__.__name__ + 'second square AE' + str(
                i) + '.csv')
        t_report += t
        print(t.__class__)
    print(scoreTotal / v)
    statisticForMetrics.append(scoreTotal / v)

    cmTotal = cmTotal / v
    print(cmTotal / v)

    # utils.plot_confusion_matrix(cmTotal, PN, item.__class__.__name__+'_PCA')
    utils.plot_confusion_matrix(cmTotal, PN, 'second square augmentation SVM 40%')
    statisticForMetrics = pd.DataFrame(statisticForMetrics)
    statisticForMetrics.to_csv('second square augmentation SVM 40%.csv')